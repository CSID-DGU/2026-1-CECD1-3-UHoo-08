...
import { getMyProfile, updateProfile, getProfileImageUrl } from "../../api/userApi";
  ...
  useEffect(() => {
    getMyProfile()
      .then((res) => {
        ...
        if (u.profileImageUrl) setPreviewUrl(getProfileImageUrl(u.profileImageUrl));
      })
      .catch(() => {});
  }, []);
  ...
  return (
    <AppLayout>
      ...
      <section className="flex flex-col px-6 pb-8 pt-10">
        ...
        {/* 이름 */}
        <div className="mt-6">
          <label className="text-body2 text-gray-500" htmlFor="name">이름</label>
          <input
            id="name"
              ...
              ([value, label]) => (
                <button
                  className={`h-[50px] rounded-lg text-body1 ${
                    gender === value ? "bg-primary-300 text-white" : "bg-gray-100 text-gray-500"
                  }`}
                  key={value}
